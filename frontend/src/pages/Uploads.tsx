...
export default function Uploads() {
  ...
  useEffect(() => {
    apiGet("/api/uploads").then(setUploads);
  }, []);
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <Button variant="danger" onClick={handleDeleteAll} disabled={uploads.length === 0}>
            Delete All
          </Button>
        }
      />
          ...
          {uploads.map((u) => (
            <Link to={`/uploads/${u.id}`} key={u.id}>
              <Card className="relative hover:shadow-md transition-shadow">
                ...
                <img src={fileUrl(u.image_path)} className="w-full aspect-video object-cover rounded-lg mb-3" />
